! function () {
	...
	var dirty = cpd.dirty,
		...
		scoreDependencies = function (sentence, token) {
			var deps = token.deps.dependencies,
				i, l = deps.length,
				s = 0,
				t;
			// If no dependencies
	...
	detectors.after('s', function (sentence, index, sentences) {
		...
		for (i = 0; i < l; i++) {
			...
			//Calculate the nearest verb tense to the end of the sentence
			if (attr.is_verb) {
				sentence.profile.nearest_verb_tense = attr.tense;
			}
